function [features, levels, indexes, gradsums] = unentanglePyramid_my(pyramid,pcsz,params) 
  conf=params;
...
% Author: saurabh.me@gmail.com (Saurabh Singh).
  ...
  %if(dsbool(conf,'whitening'))   
      features=bsxfun(@minus,features,params.datamean)*(params.whitenmat)';    
  %end
  %if(dsbool(conf,'normalizefeats'))
    ...
    features=bsxfun(@rdivide,features,sqrt(var(features,1,2).*size(features,2)));
  %end
  if(0)
