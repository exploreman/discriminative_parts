% this function is to test the performance using random initialization
% instead of k-means
...
% set parameters
params.databaseName = 'scene_categories';  %'MITindoors';%'uiucsports'; %'uiucsports'; %'willowactions';
...
%          end;
    ...
    if iswhitening == 0
        ...
        if(0)
            ...
            APT_run('ComputeHogFeature_whiten_perimg', database_para, params_whitening,featureFolderPath_para,'UseCluster', 1, 'ClusterID', 2, 'KeepTmp', 1, 'NJobs', min(60, num_class));
        end
    end
...
for round = 1 : num_rounds     
    ...
    if ~(skipTrainTestSetup == 1)    
      ...
      load(saveClusterCenters);
    end
    
    % Step 3.2: perform clustering over the patches crop from each class
    saveClusterCenters = [DictFolderPath_new,'ClusterCenters_', num2str(params.PartSize_fea), '_iswhtened_', num2str(iswhitening), '_nClass', num2str(round) ,'_randinit.mat'];
    if ~(skipInitFilters == 1)
        
        fprintf('round %d: perform clustering over the patches crop from each class', round);
        params.numClusters = 400;
        %C = APT_run('dictionaryInit_rand', tr_idx, database, params, 'UseCluster', 1,  'ClusterID', clusterID, 'KeepTmp', 1, 'NJobs', 34, 'Memory', 8000, 'Libs',{'/scratch/jisun/DownloadCodes/vlfeat-0.9.17/bin/glnxa64'} ); %'ClusterID', 1,
        
        
        Filters_Initialization = [];        
        for k = 1 : num_class
           C{k} = dictionaryInit_rand(tr_idx{k}, database, params);
           Filters_Initialization{k} = C{k};
        end        
        save(saveClusterCenters, 'Filters_Initialization', 'database_posi', 'database_nega', 'database_test', 'tr_idx', 'ts_idx');
    else
        load(saveClusterCenters);
    end
    ...
    modelfile = [DictFolderPath_new, 'LearnedParts_MSMF_', num2str(params.PartSize_fea), '_iswhtened_', num2str(iswhitening), '_Round', num2str(round) ,'_Regu_',num2str(params.lambdas),'_randinit.mat']
       
    if skipTrainFilters == 0
        if iswhitening == 0    
            [filters, vThreshs] = APT_run('train_parts_LSVM_sparsity', database_posi, database_nega, Filters_Initialization, params, optiParas, 'UseCluster', 1,'ClusterID', clusterID,  'KeepTmp', 1, 'NJobs', 34, 'HostName', {'node017', 'node002', 'node003', 'node007', 'node016'});
        else
        ...
        save(modelfile, 'Filters_Initialization', 'database_posi', 'database_nega', 'database_test','Filters_opti', 'tr_idx', 'ts_idx', 'tr_label', 'ts_label');  
        clear Filters_Initialization Filters_opti;
    else
        load(modelfile, 'Filters_Initialization', 'database_posi', 'database_nega', 'database_test','Filters_opti', 'tr_idx', 'ts_idx', 'tr_label', 'ts_label');  
        clear Filters_Initialization Filters_opti;
    end
    ...
    % save the results
    resultfile = [ResultFolderPath_new, 'Results_', num2str(params.PartSize_fea), '_iswhtened_', num2str(iswhitening), '_Round', num2str(round) ,'_Regu',num2str(params.lambdas),'_randinit.mat']   
    accu_all{round} = accu;
