...
for round = 1 : num_rounds     
    ...
    if skipTrainFilters == 0
        %if iswhitening == 0    
            [filters, vThreshs] = APT_run('train_parts_LSVM_sparsity', database_posi, database_nega, Filters_Initialization, params, optiParas, 'UseCluster', 1,'ClusterID', clusterID,  'KeepTmp', 1, 'NJobs', 34);
        %else
    ...
    %% save the results
    resultfile = [ResultFolderPath_new, 'Results_', num2str(params.PartSize_fea), '_iswhtened_', num2str(iswhitening), '_Round', num2str(round) ,'_Regu',num2str(params.lambdas),'.mat']   
    accu_all{round} = accu;
