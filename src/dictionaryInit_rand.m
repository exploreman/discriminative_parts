function Filters = dictionaryInit_rand(tr_idx, database, params) %numClusters, PartSize_fea, num_rand_per_imgs, Scales)
...
num_feats_sum = zeros(1, num_scale);
scale_sele = [1 : 2 : length(Scales) ]; %find(Scales == 1);

num_parts_all = numClusters;
...
for d = 1 : num_img
   ...
   [Part_Feat_hog, num_feats_hog, posi_x, posi_y] = cropPartsFromImages_rand(Feats_HoG(scale_sele), grids_x_hog(scale_sele)', grids_y_hog(scale_sele)', PartSize_fea, num_rand_per_imgs);
   
   for k = 1 : num_scale      
       num_feats_hog = size(Part_Feat_hog{k}, 4);
      
       Part_FeatSet{k}(:,:,:,num_feats_sum(k) + 1 : num_feats_sum(k) + num_feats_hog) = Part_Feat_hog{k};
       num_feats_sum(k) = num_feats_sum(k) + num_feats_hog; 
   end
...
if(num_scale > 0)
    for k = 1 : num_scale 
        Part_Feat_reshape = reshape(Part_FeatSet{k}, [], num_feats_sum(k));
