function [accuracy, accu]=classification_learnedParts_softmax(FileModel, saveFolder, params, round)
trainSVMonCluster = 1;
...
num_perscale = 20;
dict = genDictionary_mulcls(Filters_opti, PartSize_fea, num_perscale);
num_words = length(dict.thresholds); 
num_classes = length(database_posi);  
...
if ~(skipCoding == 1)           
    ...
    if(trainSVMonCluster == 0)
        [codeSets] = APT_run('coding_maxResp_filters', database_para, dict, PartSize_fea_para, featDims_para ,poolPara, 'UseCluster', 1, 'KeepTmp', 1,  'ClusterID', 1, 'NJobs', 40); %, 
        ...
        APT_run('coding_maxResp_filters_s2f', database_para, dict, PartSize_fea_para, featDims_para ,poolPara, 'UseCluster', 1, 'KeepTmp', 1,  'ClusterID', 2, 'NJobs', 40); %, 
    end
end
...
if((trainSVMonCluster == 0))
    ...
    model_para=APT_run('trainCluster',labels_para,data_para, 'UseCluster', 1, 'KeepTmp', 1,  'ClusterID', 2, 'NJobs', 1, 'Memory', 30000);
    model = model_para{1};
end
