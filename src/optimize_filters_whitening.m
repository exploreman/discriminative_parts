function [Filters, b, vThreshs] = optimize_filters_whitening(database_posi, database_nega, model, FPosi, params, params_whitening, optiParas)
...
while (nIter < maxIters & err > errTolerance & L < L_max)  
    ...
    % make filter
    [Filters_curr] = GroupVectors2Filters_whiten(vNParts, wk, PartSize_fea, params.featDims);
        
    % randomly select the negative training data
    fprintf('\n Prepare negative examples  \n'); 
    feats_nega = randSample_nega_whitening(database_nega, Filters_curr, params,params_whitening, num_nega_rand, vNParts);
    
    % randomly select the positive training data
    fprintf('\n Prepare positive examples  \n');
    feats_posi = randSample_posi_whitening(database_posi, FPosi, params, params_whitening, num_posi_rand, vNParts);
...
Filters = GroupVectors2Filters_whiten(vNParts, xk, PartSize_fea, featDims);
b = xk(end-num_parts);
